...
import li.cil.oc.api.network.{Analyzable, Component, Node, SidedEnvironment, Visibility}
...
class Screen(var tier: Int) extends TileEntity(Blocks.TILE_ENTITY_SCREEN) with traits.TextBuffer with SidedEnvironment with traits.Rotatable with traits.RedstoneAware with traits.Colored with Analyzable with Ordered[Screen] with IForgeTileEntity {
  def this() = this(0)
  ...
  /**
   * @TODO Keyboard - Uncomment
   */
  override def sidedNode(side: Direction): Node = null // if (side != facing || (getWorld.isBlockLoaded(getPos.offset(side)) && getWorld.getTileEntity(getPos.offset(side)).isInstanceOf[Keyboard])) node else null
...
//      case (side, keyboard: Keyboard) => keyboard.hasNodeOnSide(side.getOpposite)
  ...
  def toScreenCoordinates(hitX: Double, hitY: Double, hitZ: Double): (Boolean, Option[(Double, Double)]) = {
    ...
    def dot(f: Direction) = f.getXOffset * hitX + f.getYOffset * hitY + f.getZOffset * hitZ
  ...
  override def updateEntity() {
    ...
    if (shouldCheckForMultiBlock && ((isClient && isClientReadyForMultiBlockCheck) || (isServer && isConnected))) {
      ...
      while (pending.nonEmpty) {
        ...
        /**
         * TODO This method seems like it doesn't exist anymore...
         */
        if (isClient) {
...
//            bounds.maxX.toInt, bounds.maxY.toInt, bounds.maxZ.toInt)
    ...
    if (arrows.nonEmpty) {
      for (arrow <- arrows) {
        val hitX = arrow.getPosX - x
        val hitY = arrow.getPosY - y
        val hitZ = arrow.getPosZ - z
        arrow.func_234616_v_ match {
...
//    }
  ...
  @OnlyIn(Dist.CLIENT)
  override def getMaxRenderDistanceSquared: Double = if (isOrigin) super.getMaxRenderDistanceSquared else 0
  ...
  private def project(t: Screen) = {
    def dot(f: Direction, s: Screen) = f.getXOffset * s.x + f.getYOffset * s.y + f.getZOffset * s.z
  ...
  private def unproject(x: Int, y: Int, z: Int) = {
    def dot(f: Direction) = f.getXOffset * x + f.getYOffset * y + f.getZOffset * z
