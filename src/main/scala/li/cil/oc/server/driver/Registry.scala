...
import li.cil.oc.api.driver.{Converter, DriverBlock, DriverItem, EnvironmentProvider, InventoryProvider}
...
//      case arg: Product => convertList(arg, arg.productIterator.zipWithIndex, memo)
  ...
  def convertMap(obj: AnyRef, map: Map[_, _], memo: util.IdentityHashMap[AnyRef, AnyRef]): AnyRef = {
    val converted = memo.getOrDefault(obj, mutable.Map.empty[AnyRef, AnyRef]) match {
    ...
    map.foreach {
      ...
      case _ => println("Convert to map: This shouldn't have triggered!")
    }
//    map collect {
//      case (key: AnyRef, value: AnyRef) => converted += convertRecursively(key, memo) -> convertRecursively(value, memo)
//    }
