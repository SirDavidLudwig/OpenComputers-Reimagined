...
trait VirtualFileSystem extends OutputStreamFileSystem {
  ...
  protected class VirtualFile extends VirtualObject {
    ...
    override def load(nbt: CompoundNBT): Unit = {
      super.load(nbt)
      data.clear()
      data ++= nbt.getByteArray("data")
    }

    override def save(nbt: CompoundNBT): Unit = {
      super.save(nbt)
    ...
    override def read(b: Array[Byte], off: Int, len: Int) =
      if (!isClosed) {
        ...
        if (count == 0) -1
          ...
          file.data.slice(position, file.data.length).copyToArray(b, off, n)
          position += n
          n
        }
      }
