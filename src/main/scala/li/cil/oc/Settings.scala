...
//  val nanomachineHudPos = Array.from(config.getDoubleList("client.nanomachineHudPos"): _*) match {
//    case Array(x, y) =>
//      (x: Double, y: Double)
//    case _ =>
...
//  val cpuComponentSupport = Array.from(config.getIntList("computer.cpuComponentCount"): _*) match {
...
//      Array(8, 12, 16, 1024)
//  }
//  val callBudgets = Array.from(config.getDoubleList("computer.callBudgets"): _*) match {
...
//  val ramSizes = Array.from(config.getIntList("computer.lua.ramSizes"): _*) match {
...
//  val upgradeFlightHeight = Array.from(config.getIntList("robot.upgradeFlightHeight"): _*) match {
...
//  val bufferCapacitorUpgrades = Array.from(config.getDoubleList("power.buffer.batteryUpgrades"): _*) match {
//    case Array(tier1, tier2, tier3) =>
//      Array(tier1: Double, tier2: Double, tier3: Double)
//    case _ =>
...
//  val wirelessCostPerRange = Array.from(config.getDoubleList("power.cost.wirelessCostPerRange"): _*) match {
//    case Array(tier1, tier2) =>
...
//  val caseRate = (Array.from(config.getDoubleList("power.rate.case"): _*) match {
//    case Array(tier1, tier2, tier3) =>
//      Array(tier1: Double, tier2: Double, tier3: Double)
//    case _ =>
...
//  val hddSizes = Array.from(config.getIntList("filesystem.hddSizes"): _*) match {
//    case Array(tier1, tier2, tier3) =>
...
//  }
//  val hddPlatterCounts = Array.from(config.getIntList("filesystem.hddPlatterCounts"): _*) match {
//    case Array(tier1, tier2, tier3) =>
//      Array(tier1: Int, tier2: Int, tier3: Int)
//    case _ =>
...
//  val httpHostWhitelist = Array(config.getStringList("internet.whitelist").map(new Settings.AddressValidator(_)): _*)
  ...
  // ----------------------------------------------------------------------- //
//  // hologram
//  val hologramMaxScaleByTier = Array.from[Double]((config.getDoubleList("hologram.maxScale"): _*).toArray()) match {
//    case Array(tier1, tier2) =>
...
//      Array(3.0, 4.0)
//  }
//  val hologramMaxTranslationByTier = Array.from[Double](config.getDoubleList("hologram.maxTranslation"): _*) match {
//    case Array(tier1, tier2) =>
//      Array((tier1: Double) max 0.0, (tier2: Double) max 0.0)
//    case _ =>
...
//  val screenDepthsByTier: Array[api.internal.TextBuffer.ColorDepth] = Array(api.internal.TextBuffer.ColorDepth.OneBit, api.internal.TextBuffer.ColorDepth.FourBit, api.internal.TextBuffer.ColorDepth.EightBit)
  ...
  def load(file: File) = {
    ...
    val defaults = {
      ...
      val config = Source.fromInputStream(in)(Codec.UTF8).getLines().mkString("", System.lineSeparator, System.lineSeparator)
      in.close()
      ConfigFactory.parseString(config)
    }
    val config =
      try {
        val plain = Source.fromFile(file)(Codec.UTF8).getLines().mkString("", System.lineSeparator, System.lineSeparator)
