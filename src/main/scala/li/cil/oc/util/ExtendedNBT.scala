...
//      case Some(v: java.util.Map[K, _]@unchecked) => mapAsScalaMap(v).toMap
  ...
  implicit def extendNBTBase(nbt: INBT): ExtendedNBTBase = new ExtendedNBTBase(nbt)
  ...
  class ExtendedNBTBase(val nbt: INBT) {
    def toTypedMap: Map[String, _] = Map("type" -> nbt.getId, "value" -> (nbt match {
      case tag: ByteNBT => tag.getByte
      case tag: ShortNBT => tag.getShort
      case tag: IntNBT => tag.getInt
      case tag: LongNBT => tag.getLong
      case tag: FloatNBT => tag.getFloat
      case tag: DoubleNBT => tag.getDouble
      case tag: ByteArrayNBT => tag.getByteArray
      case tag: StringNBT => tag.getString
      case tag: ListNBT => tag.map((entry: INBT) => entry.toTypedMap)
//      case tag: CompoundNBT => tag.keySet().collect {
//        case key: String => key -> tag.get(key).toTypedMap
//      }.toMap
      case tag: IntArrayNBT => tag.getIntArray
      case _ => throw new IllegalArgumentException()
    }))
  }
  ...
  class ExtendedCompoundNBT(val nbt: CompoundNBT) {
    ...
    def getDirection(name: String) = {
      nbt.getByte(name) match {
        ...
        case id => Option(Direction.byIndex(id))
      }
    }
