...
import net.minecraftforge.items.{CapabilityItemHandler, IItemHandler, IItemHandlerModifiable}
...
object InventoryUtils {
  ...
  def haveSameItemType(stackA: ItemStack, stackB: ItemStack, checkNBT: Boolean = false): Boolean =
    !stackA.isEmpty && !stackB.isEmpty &&
      ...
      (stackA.getDamage == stackB.getDamage) && /** WARN Removed check for subtyes since they aren't a thing anymore*/
      (!checkNBT || ItemStack.areItemStackTagsEqual(stackA, stackB))

  /**
   * @WARN Capability here could be implemented wrong...
   *
  ...
  def inventoryAt(position: BlockPosition, side: Direction): Option[IItemHandler] = position.world match {
    case Some(world) if world.blockExists(position) => world.getTileEntity(position) match {
      case tile: TileEntity if tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side).isPresent =>
        Option(tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side).orElse(null))
//      case tile: IInventory => Option(asItemHandler(tile, side))
      case _ => world.getEntitiesWithinAABB(classOf[Entity], position.bounds).asScala
        .filter((e: Entity) => e.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side).isPresent
          && (!e.isLiving || (e.isInstanceOf[LivingEntity] && e.isAlive)))
